...
모델이 자유롭게 분류명을 생성하지 않습니다. 카탈로그에 등록된 `major::minor` 키만 선택할 수 있으며, 카탈로그를 바꾸면 JSON Schema와 프롬프트의 허용 목록도 함께 바뀝니다.

### 이중 계약 검증

strict JSON Schema가 응답의 필드·타입·허용값을 제한합니다. 그다음 서버가 대표 분류와 추가 분류의 중복, 입력 이름 불일치, 확정 결과의 대표 분류 누락처럼 JSON Schema만으로 표현하기 어려운 관계를 다시 검사합니다.

### 검수 우선 결과

분류 결과에는 카테고리뿐 아니라 `needs_review`와 판정 사유가 포함됩니다. 모호한 입력은 억지로 확정하지 않으며, 가장 유력한 후보가 있으면 사람 검수용 제안값으로 보존합니다.

### 도메인 설정 격리

분류 카테고리와 프롬프트는 코드가 아니라 로컬 설정 파일로 주입합니다. 저장소에는 합성 예시만 포함하므로 실제 도메인 데이터와 정책을 공개하지 않고 같은 서버 구조를 다른 분류 문제에 적용할 수 있습니다.

### 로컬 검증 하네스

운영 서비스에 LLM 분류를 연결하기 전에 프롬프트, 카탈로그, 출력 계약을 HTTP API로 실행하고 검증하는 용도입니다. 인증·권한과 운영 데이터 반영은 이 서버의 책임이 아닙니다.
